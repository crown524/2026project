"""GBDT 超参数搜索脚本（LOSO on train only）

搜索范围：min_samples_leaf, max_features, n_estimators
评估协议：仅在 train 集（107 人）内做留一交叉验证，dev 集完全隔离。

用法：
    python tune_gbdt.py --lanes obs
    python tune_gbdt.py --lanes tt,obs
    python tune_gbdt.py --lanes obs,ac --top 5
"""
...
def main() -> int:
    ...
    n_tr = len(train_df)
    print(f"lanes={lanes}  train 样本: {n_tr}")
    if n_tr < 20:
    ...
    for i, vals in enumerate(combos, 1):
        ...
        print(f"  [{i:2d}/{len(combos)}] "
              ...
              f"max_feat={str(params['max_features']):5s}  "
    ...
    print(f"\n下一步：用以上参数在 run_experiment.py 的 make_model() 中替换默认值，")
    print(f"再用 --model gbdt 重跑消融实验确认 dev 结果。")
    return 0
